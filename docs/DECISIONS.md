...
## ADR-017 — Core Value Quest는 경험 여부만 기록한다
...
Core Value Store에는 경험한 Value ID와 12개 완료 후 선택한 첫 주 실천 Value ID만 저장한다. Scenario에서 고른 선택지는 Modal의 일시적인 UI 상태로만 사용하고 Store에 결과, 점수 또는 판정으로 보존하지 않는다. 같은 Value의 경험 기록은 한 번만 추가한다.
...
## ADR-020 — 공장성은 World 확대보다 산업 디테일로 강화한다
...
기존 34×30 World와 Zone 좌표를 유지하면서 concrete 작업면, 물류·안전 노면, fence, retaining wall, loading door, pallet, Vision Camera와 Monitoring Screen을 코드 기반 geometry로 추가한다. 자연물은 삭제하지 않고 외곽의 보조 요소로 유지한다.
