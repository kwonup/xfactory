...
5. 각 커밋 작업이 끝나면 반드시:
   ...
   - build 가능 여부
   를 확인한다.
6. 검증이 통과하면 Git commit을 만든다.
7. 커밋 후에는 **작업을 멈추고 사용자 검토를 기다린다.**
8. 사용자가 `다음 커밋 진행`이라고 명시적으로 요청하기 전에는 다음 단계로 넘어가지 않는다.
...
# 35. Codex의 각 커밋 종료 보고 형식

각 커밋 완료 후 반드시 아래 형식으로 보고하고 멈춘다.
...
사용자 검토 후 `다음 커밋 진행` 요청을 기다립니다.
```

검증 실패가 있으면 커밋하지 말고 먼저 원인을 해결한다.
